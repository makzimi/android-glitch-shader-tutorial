...
import dev.maxkach.shaders.product.redTintShader
import kotlin.math.absoluteValue

@Composable
fun RedTintSampleScreen(
    onBackPressed: () -> Unit,
    modifier: Modifier = Modifier
) {
    var selectedImage by remember { mutableIntStateOf(0) }
    var selectedColor by remember { mutableIntStateOf(0) }
    val state = ProductCardStateCreator.create(selectedImage, selectedColor)
    Scaffold(
        modifier = modifier,
        topBar = { TopBar("Red Tint Shader", onBackPressed) },
